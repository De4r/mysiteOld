...
def singe_slug(request, single_slug):
    ...
    if single_slug in categories:
        ...
        for m in matching_series.all():
            part_one = Tutorial.objects.filter(tutorial_series__tutorial_series=
                                               m.tutorial_series).earliest("tutorialPublished")
    ...
    if single_slug in tutorials:
        ...
        tutorials_from_series = Tutorial.objects.filter(tutorial_series__tutorial_series=
                                                        this_tutorial.tutorial_series).order_by("tutorialPublished")
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.info(request, f"Logged in to account: {username}")
